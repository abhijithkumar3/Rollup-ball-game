...
let update = () =>
{
    ...
    let sky = c.createLinearGradient(0, 0, 0, a.height);
    sky.addColorStop(0, '#020712');
    sky.addColorStop(.28, '#0a2140');
    sky.addColorStop(.62, '#2e6ea5');
    sky.addColorStop(1, '#dfefff');
    c.fillStyle = sky;
    c.fillRect(0, 0, a.width, a.height);

    for(i=120; i--;)
    {
        c.fillStyle = `rgba(255,255,255,${0.2 + (i % 5) * 0.16})`;
        c.fillRect((i * i + z * i / 99) % a.width, (i ** 3.1 + z * 1.5) % (a.height * 0.8), i % 3 + 1, i % 3 + 1);
    }
